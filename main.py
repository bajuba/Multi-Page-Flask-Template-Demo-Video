from flask import Flask, render_template, request, session
from random import choice

app = Flask(__name__)
app.secret_key = 'hellogoodbye'


number_list = [
	100, 101, 200, 201, 202, 204, 206, 207, 300, 301, 302, 303, 304, 305, 307, 400, 401, 402, 403, 404, 405, 406, 408, 409, 410, 411, 412, 413, 414, 415,
	416, 417, 418, 421, 422, 423, 424, 425, 426,
	429, 431, 444, 450, 451, 500, 502, 503, 504, 506, 507, 508, 509, 510, 511, 599
]

@app.route('/')
def index():
	return render_template('index.html')

@app.route('/user/',methods = ['POST', 'GET'], defaults={'username': None})
@app.route('/user/<username>')
def generate_user(username):
  if not username:
    #POST
    username = request.form['username']
    fruit = request.form['fruit']
    print(request.form)
    #GET
    #username = request.args.get('username')

  if not username:
    return 'Sorry error something, malformed request.'
  session['username'] = username
  #username = session['username']
  return render_template('personal_user.html', user=username, fruit=fruit, list=number_list)

@app.route('/page')
def random_page():
  if request.args.get('logout'):
    request.args.get('username')
    session.pop('username', None)
    return 'You have Logged Out. =)'
  return render_template('page.html', code=choice(number_list) ,username=session['username'])

app.run(host='0.0.0.0', port=8080)